...
# --- FIREBASE SETUP (Direct Secrets Access) ---
if not firebase_admin._apps:
    try:
        # TOML format se direct dictionary ban rahi hai
...
try:
    ...
    st.warning("Database connect nahi hua. Scans save nahi honge.")
...
with tab1:
    ...
    if not uploaded_file:
        uploaded_file = st.camera_input("Ya Photo Lein")
        ...
        if st.button("🚀 Process & Save"):
            with st.spinner("AI Analysis kar raha hai..."):
                try:
                    ...
                    st.download_button("📥 Download PDF", data=pdf_data, file_name="Analysis_Report.pdf")
...
with tab2:
    ...
    try:
        scans = db.collection('scans').order_by('timestamp', direction=firestore.Query.DESCENDING).limit(5).stream()
        for scan in scans:
            data = scan.to_dict()
            with st.expander(f"Scan - {data['timestamp'].strftime('%H:%M %d/%m')}"):
                st.markdown(data['summary'])
    except:
        st.info("Abhi tak koi scans nahi hain.")
